...
export function SidebarThread({thread, isActive}: {thread: Thread; isActive: boolean}) {
	...
	if (isEditing) {
		return (
			<form onSubmit={handleRenameSubmit} className="flex items-center gap-2 px-3 py-2 bg-neutral-800 rounded-lg">
				...
				<button type="button" onClick={() => setIsEditing(false)} className="text-neutral-500 hover:text-neutral-300">
					<X className="w-4 h-4" />
				</button>
			</form>
		);
		...
		<div className="relative" ref={menuRef}>
			<button
				...
				<div
					...
					className={`p-1 rounded hover:bg-neutral-700 transition-colors ${isMenuOpen ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
			...
			{/* The Dropdown Menu */}
			{isMenuOpen && (
				<div className="absolute right-0 top-10 w-40 bg-neutral-800 border border-neutral-700 rounded-lg shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
					<button
						onClick={(e) => {
							e.stopPropagation();
							handleShare();
						}}
						className="w-full flex items-center gap-2 px-3 py-2 text-xs text-neutral-300 hover:bg-neutral-700 hover:text-white transition-colors">
					...
					<button
						...
						className="w-full flex items-center gap-2 px-3 py-2 text-xs text-red-400 hover:bg-neutral-700 hover:text-red-300 transition-colors">
